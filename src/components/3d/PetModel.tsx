...
export const PetModel: React.FC<PetModelProps> = ({ textureUrl, animation, dayNightMode = 'day', onTextureLoadState }) => {
  const outerGroupRef = useRef<THREE.Group>(null);
  
  const activeAnimation = useMemo(() => {
     if (!animation) return null;
     if ('day' in animation) return (animation as DayNightAnimation)[dayNightMode];
     return animation as PetAnimation;
  ...
  const precomputedMaterials = useMemo(() => {
    return textures.map((t) => {
      ...
      const createMaterials = (offsetX: number, offsetY: number) => {
        const mats = [];
        const regions = [
          { x: offsetX + 16, y: offsetY + 8, w: 8, h: 8 }, // Right Side
          { x: offsetX + 0,  y: offsetY + 8, w: 8, h: 8 }, // Left Side
          { x: offsetX + 8,  y: offsetY + 0, w: 8, h: 8 }, // Top
          { x: offsetX + 16, y: offsetY + 0, w: 8, h: 8 }, // Bottom
          { x: offsetX + 8,  y: offsetY + 8, w: 8, h: 8 }, // Front
          { x: offsetX + 24, y: offsetY + 8, w: 8, h: 8 }, // Back
        ];

        const texHeight = (matTex.image as any)?.height || 64;
        const texWidth = (matTex.image as any)?.width || 64;
